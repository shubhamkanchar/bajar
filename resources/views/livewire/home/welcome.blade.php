                ...
                <div class="d-md-flex position-relative justify-content-center mb-4">
                    <div class="bg-white rounded-5 p-2 border me-2" x-data="{ focused: false }"
                        ...
                        <input class="form-select rounded-5 mt-1 border border-0 bg-white" name="city"
                            placeholder="City" autocomplete="off" @focus="focused = true" @blur="focused = false"
                            wire:click="toggle" wire:model.live="selectedCity" />
                    </div>
                    @if ($isOpen)
                    ...
                    <div class="d-flex align-items-center justify-content-between bg-white rounded-5 p-2 border mt-2 mt-md-0"
                        ...
                        <input wire:model="productName" class="form-control border border-0 rounded-5 me-2 bg-white"
                            type="text" placeholder="Search" autocomplete="off" @focus='focused = true'
                            @blur="focused = false" wire:keyup="searchRestart()">
                ...
                <div class="container py-4">
                    ...
                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 g-4">
                        @foreach ($sellers as $seller)
                            @php
                                // Generate random banner once per seller
                                $randomBanner = rand(1, 8);
                            @endphp
                            <div class="col">
                                <div class="card h-100 shadow-sm">
                                    <div class="ratio ratio-21x9">
                                        @if ($seller->bg_image)
                                            <img src="{{ asset('storage/' . $seller->bg_image) }}"
                                                class="card-img-top h-100" alt="{{ $seller->name }}" loading="lazy">
                                        @else
                                            <picture>
                                                <source media="(max-width: 767px)"
                                                    srcset="{{ asset('assets/image/mobile/banner_0' . $randomBanner . '.png') }}">
                                                <img class="card-img-top h-100"
                                                    src="{{ asset('assets/image/desktop/banner_0' . $randomBanner . '.png') }}"
                                    ...
                                    <div class="card-body">
                                        ...
                                        <p class="mb-0 small">
                                            ...
                                            {{ $seller->address->full_address ?? 'Address not available' }}
                                        </p>
                                    </div>
                                ...
                                @foreach ($ads as $ad)
                                    <li class="splide__slide p-2">
                                        <img class="w-100" src="{{ asset('storage/' . $ad->image) }}"
                                            alt="Advertisement" loading="lazy">
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                                ...
                                <div class="border rounded position-relative rounded-3">
                                    <div class="ratio ratio-16x9">
                                        <img src="{{ asset('storage/' . $data->blog_image) }}"
                                            class="d-block w-100 rounded-3" alt="{{ $data->title }}"
                                            loading="lazy">
                                    </div>
                                    <div style="height: 140px">
                                        <div class="p-1 fw-bold text-title">{{ $data->title }}</div>
                                        <!-- FIX: Sanitized blog description -->
    ...
    <script>
        ...
        async function success(position) {
            ...
            try {
                ...
                Livewire.dispatch('selectedStatefun', {
                    'value': state
                });
            } catch (err) {
