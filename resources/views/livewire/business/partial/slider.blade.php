            ...
            <div class="row">
                <div class="col-md-5 mb-3 position-relative">
                    @if ($product_images['product_image1'])
                        <button type="button" style="z-index: 1"
                            class="btn btn-danger position-absolute top-0 end-1 m-1"
                            wire:click="removeImage('product_image1')" wire:key="remove-button-1">
                            <i class="fa fa-times"></i>
                        </button>
                    @endif
                    <div class="dashed-border ratio ratio-1x1">
                        @if ($isEdit && gettype($product_images['product_image1']) == 'string')
                            <img src="{{ asset('storage/' . $product_images['product_image1']) }}"
                                class="img-fluid">
                        @elseif ($product_images['product_image1'])
                            <img src="{{ $product_images['product_image1']->temporaryUrl() }}" class="img-fluid">
                        @else
                            <span class="text-center" style="top: 35%;" wire:loading.remove
                                wire:target="product_images.product_image1">
                                <input type="file" wire:model.blur="product_images.product_image1"
                                    id="productImage1" hidden accept="image/*">
                    ...
                    @error('product_images.product_image1')
                        ...
                        @foreach ([2, 3, 4, 5, 6] as $index)
                            <div class="col-md-4 mb-3 col-6 position-relative">
                                @if ($product_images['product_image' . $index])
                                    <button type="button" style="z-index: 1"
                                        class="btn btn-danger position-absolute top-0 end-1 m-1"
                                        wire:click="removeImage('product_image{{ $index }}')"
                                ...
                                <div class="dashed-border ratio ratio-1x1">
                                    ...
                                    @elseif($product_images['product_image' . $index])
                                        <img src="{{ $product_images['product_image' . $index]->temporaryUrl() }}"
                                            class="img-fluid">
                                    @else
                                        <span class="text-center" style="top: 35%;" wire:loading.remove
                                            wire:target="product_images.product_image{{ $index }}">
                                            <input type="file"
                                ...
                                @error('product_images.product_image' . $index)
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        @endforeach
                ...
                <div class="col-md-6" x-data="{ showPrice: @entangle('showPrice') }">
                    <div class="mt-2 d-flex justify-content-between bg-white rounded-5 p-2 border">
                        <button type="button" class="btn rounded-5 pt-2 pb-2 w-100"
                            :class="{ 'btn-dark': showPrice, 'btn-light': !showPrice }" data-item="email"
                        ...
                        <!-- Hide Price Button -->
                        <button type="button" class="btn rounded-5 pt-2 pb-2 w-100"
                            :class="{ 'btn-dark': !showPrice, 'btn-light': showPrice }" data-item="phone"
                ...
                <div class="col-md-12 mt-4">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5 mt-2 mb-2">
                            @if ($isEdit)
                                <button wire:click.prevent="deleteProduct" wire:loading.attr="disabled"
                                    wire:confirm="Are you sure want to delete this product"
